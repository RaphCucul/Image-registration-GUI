                          ...
                          bool video)
{
    ...
    else
    {
        ...
        /*postProcessing(i_cap,
                       ...
                       i_cutoutStandard,
                       i_cutoutExtra,
                       adjustedCutoutStandard,
                       fullyRegistratedFrame,
                       i_areaMaximum,i_coordsFrangiStandardReferencialReverse,
                       _tempPOCX,_tempPOCY,_frangiX,_frangiY,_frangiEucl,_pocX,_pocY,i_frangiParameters,
                       _tempAngles,_maxAngles,i_margins,video);*/
...
bool postProcessing(cv::VideoCapture i_cap,
    ...
    if (i_scaleChanged){
        i_referencialFrame(i_cutoutExtra).copyTo(_referentialSubstituion);
        _referentialSubstituion(i_adjustedCutoutStandard).copyTo(referencialCutout);
    }
    else{
        i_referencialFrame.copyTo(_referentialSubstituion);
        _referentialSubstituion(i_cutoutStandard).copyTo(referencialCutout);
    ...
    else
    {
        ...
        if (!translationCorrection(fullyRegistratedFrame,
                                   _referentialSubstituion,
                                   fullyRegistratedFrame_correction,
                                   i_scaleChanged ? i_adjustedCutoutStandard : i_cutoutStandard,
                                   correction,
                                   i_areaMaximum)){
            qWarning()<<"Frame "<<i_index_moved<<" - registration correction failed";
            return false;
            ...
            if (!i_scaleChanged)
                ...
                interresult32f(i_adjustedCutoutStandard).copyTo(interresult32f_vyrez);
