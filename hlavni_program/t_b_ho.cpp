...
void t_b_HO::chosenPath(int i_index)
 {
     if (QObject::sender() == ui->Combo_videoPath)
         SharedVariables::getSharedVariables()->setPath("cestaKvideim",typeLists["cestaKvideim"].at(i_index));
     if (QObject::sender() == ui->Combo_savingVideo)
         SharedVariables::getSharedVariables()->setPath("ulozeniVidea",typeLists["ulozeniVidea"].at(i_index));
     if (QObject::sender() == ui->Combo_VideoDataLoad)
         SharedVariables::getSharedVariables()->setPath("adresarTXT_nacteni",typeLists["adresarTXT_nacteni"].at(i_index));
     if (QObject::sender() == ui->Combo_FrangiParams)
         SharedVariables::getSharedVariables()->setPath("adresarTXT_ulozeni",typeLists["adresarTXT_ulozeni"].at(i_index));
     if (QObject::sender() == ui->Combo_FrangiParams){
         SharedVariables::getSharedVariables()->setPath("parametryFrangiFiltr",typeLists["parametryFrangiFiltr"].at(i_index));
         if (!SharedVariables::getSharedVariables()->processFrangiParameters(typeLists["parametryFrangiFiltr"].at(i_index))){
             localErrorDialogHandling[ui->Combo_FrangiParams]->evaluate("left","softError",2);
             localErrorDialogHandling[ui->Combo_FrangiParams]->show();
         }
         else
