#include "image_analysis/frangi.h"
#include <iostream>
#include <fstream>
...
void frangi2d_hessian(const Mat &src, Mat &Dxx, Mat &Dxy, Mat &Dyy, float sigma){
    ...
    int n_kern_x = int(2*round(3*sigma) + 1);
	...
	for (int x = -round(3*sigma); x <= round(3*sigma); x++){
		...
		for (int y = -round(3*sigma); y <= round(3*sigma); y++){
			kern_xx_f[i*n_kern_y + j] = 1.0f/(2.0f*CV_PI*sigma*sigma*sigma*sigma) * (x*x/(sigma*sigma) - 1) * exp(-(x*x + y*y)/(2.0f*sigma*sigma));
			kern_xy_f[i*n_kern_y + j] = 1.0f/(2.0f*CV_PI*sigma*sigma*sigma*sigma*sigma*sigma)*(x*y)*exp(-(x*x + y*y)/(2.0f*sigma*sigma));
			j++;
		}
		i++;
	}
